...
"""
NOTES (to be removed). 
Added by Marcin Wojnarski:
- Model settings can be specified as Model.__xxx__, not only as Meta subclass (more convenient for users; easy inheritance) - see BaseModel.__new__()
...
- global 'database' variable (initialized to SQLite) removed - made a lot of mess, never knew when interacting with own DB and when with the SQLite default 
...
- errors during Database.execute() now wrapped up in a PeeweeException, together with 'sql' and 'params' objects for easier debugging
- Timestamp DB type added - in MySQL that's a different type that DateTime (!). In particular, CURRENT_TIMESTAMP doesn't work for DateTimes
- CURRENT_TIMESTAMP implemented as a possible default value; for now as a client-side default, must be changed into server-side in the future
- comments and Python docs added in several places (wish there were more of them!)  
"""
...
class PeeweeException(Exception):
    "Base class for all exceptions generated by peewee."
    #def __init__(self, msg=None, sql=None):
        
class DoesNotExist(PeeweeException):
    "???"
class EmptyResultException(PeeweeException):
    ""
...
def quoteStr(s):
    """Quote the string with "..." and encode special chars inside"""
    if isinstance(s, Token): return s
    return '"' + s.replace('\\', r'\\').replace('"', r'\"') + '"'
def quote(x):
    """Quote any value 'x': turn it into string and if it was a string originally, apply escaping and "..." quoting"""
    if isinstance(x, basestring): return quoteStr(x)
    return str(x)

class Token(str):
    "Token: a special string that represents SQL construct and shall not be quoted during query generation. For example, CURRENT_TIMESTAMP used in place of default value"

CURRENT_TIMESTAMP = Token("CURRENT_TIMESTAMP")


########################################################################################################################################################
###  Adapters - connectors between abstract Python "Database" class and specific drivers of DB engines
###
...
class Database(object):
    ...
    def connect(self):
        with self._conn_lock:
            if self.deferred:
                raise PeeweeException('Error, database not properly initialized before opening connection')
    ...
    def close(self):
        with self._conn_lock:
            if self.deferred:
                raise PeeweeException('Error, database not properly initialized before closing connection')
    ...
    def execute(self, sql, params=None, require_commit=True):
        try:
            ...
            raise PeeweeException(e, sql, params)
...
class MySQLDatabase(Database):
    def __init__(self, database, **connect_kwargs):
        """Takes connection settings as separate arguments, named: host, user, passwd (not password!).
        Database name passed as 1st, unnamed argument.
        """
...
class Node(object):
    "For building abstract query expressions"
...
class Field(object):
    ...
    Uses instances of Column internally, but extends them with semantics: is a given column a primary key? foreign key? ... 
...
class BaseModel(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if _meta.pk_name is None:
            _meta.pk_name = name = 'id'
            pk = PrimaryKeyField()
            #pk.add_to_class(cls, name)
            pk.bind(cls, name)
            setattr(cls, name, pk)
            _meta.fields[name] = pk
...
class Model(object):
    ...
    # default database to be used by subclasses if they don't specify own __database__;
    # in your app, override this property (Model.__database__ = ...) after you create a Database object
    __database__ = None
    ...
    @classmethod
    def recreate_table(cls, fail_silently=False):
        "Drop the table, then create again."
        cls.drop_table(fail_silently)
        cls.create_table(fail_silently)
